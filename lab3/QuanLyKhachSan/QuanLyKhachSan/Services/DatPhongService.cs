...
namespace QuanLyKhachSan.Services
{
    ...
    {
        public DataTable LayKhach() { return Db.Query("SELECT * FROM KhachHang ORDER BY HoTen"); }
        public DataTable LayPhong() { return Db.Query("SELECT p.*,k.TenKhuVuc FROM Phong p JOIN KhuVuc k ON p.MaKhuVuc=k.MaKhuVuc ORDER BY p.SoPhong"); }
        public DataTable LayPhieuDat() { return Db.Query("SELECT d.*,k.HoTen FROM PhieuDatPhong d JOIN KhachHang k ON d.MaKhach=k.MaKhach ORDER BY d.NgayLap DESC"); }

        public DataTable LayChiTiet(string so)
        {
            return Db.Query("SELECT c.*,p.SoNguoiToiDa,p.DonGiaNgay FROM ChiTietDatPhong c JOIN Phong p ON c.SoPhong=p.SoPhong WHERE c.SoPhieuDat=@s", new SqlParameter("@s", so));
        }

        public DataTable LayNguoiLuuTru(string so)
        {
            return Db.Query("SELECT * FROM NguoiLuuTru WHERE SoPhieuDat=@s ORDER BY SoPhong, MaNguoiLT", new SqlParameter("@s", so));
        }
        ...
        public KetQuaXuLy ThemKhach(string ma, string ten, string cmnd, string qt, string sdt)
        {
            ...
            try
            {
                Db.Execute("INSERT INTO KhachHang VALUES(@m,@t,@c,@q,@s)", new SqlParameter("@m", ma), new SqlParameter("@t", ten), new SqlParameter("@c", cmnd), new SqlParameter("@q", qt), new SqlParameter("@s", sdt));
        ...
        {
            var cmd = new SqlCommand(@"SELECT COUNT(*) FROM ChiTietDatPhong c JOIN PhieuDatPhong d ON c.SoPhieuDat=d.SoPhieuDat WHERE c.SoPhong=@p AND d.TrangThai IN (N'Đã đặt', N'Đang ở') AND @nhan<=d.NgayTraDuKien AND @tra>=d.NgayNhan", cn, tx);
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    foreach (var x in ds)
                    {
                        ...
                        if (x.SoNguoi <= 0 || x.SoNguoi > Convert.ToInt32(o)) return KetQuaXuLy.Fail("Số người của phòng " + x.SoPhong + " vượt sức chứa.");
                        if (PhongTrungLich(cn, tx, x.SoPhong, nhan, tra)) return KetQuaXuLy.Fail("Phòng " + x.SoPhong + " bị trùng lịch đặt.");
                    }
        ...
        public KetQuaXuLy ThemNguoiLuuTru(string so, string phong, string ten, string cmnd, string qt)
        {
            ...
            try
            {
                ...
                Db.Execute("INSERT INTO NguoiLuuTru(SoPhieuDat, SoPhong, HoTen, SoCMND, QuocTich) VALUES(@s,@p,@t,@c,@q)", new SqlParameter("@s", so), new SqlParameter("@p", phong), new SqlParameter("@t", ten), new SqlParameter("@c", cmnd), new SqlParameter("@q", qt));
    ...
    // Class phụ trợ để chứa danh sách phòng khách chọn
