...
namespace QuanLyKhachSan.Services
{
    public class PhongTienNghiService
    {
        ...
        public DataTable LayTienNghi() { return Db.Query("SELECT t.*, l.TenLoaiTN FROM TienNghi t JOIN LoaiTienNghi l ON t.MaLoaiTN=l.MaLoaiTN ORDER BY t.MaTienNghi"); }

        public DataTable LayLapDat() { return Db.Query("SELECT p.*, l.TenLoaiTN FROM PhieuLapDat p JOIN TienNghi t ON p.MaTienNghi = t.MaTienNghi JOIN LoaiTienNghi l ON t.MaLoaiTN = l.MaLoaiTN ORDER BY NgayLap DESC"); }
        ...
        public KetQuaXuLy ThemPhong(string so, string khu, int max, decimal gia)
        {
            ...
            try
            {
                Db.Execute("INSERT INTO Phong(SoPhong, MaKhuVuc, SoNguoiToiDa, DonGiaNgay, TrangThai) VALUES(@s,@k,@m,@g,N'Trống')",
                    new SqlParameter("@s", so), new SqlParameter("@k", khu), new SqlParameter("@m", max), new SqlParameter("@g", gia));
        ...
        public KetQuaXuLy ThemTienNghi(string ma, string loai, int stt, string tt)
        {
            ...
            try
            {
                Db.Execute("INSERT INTO TienNghi VALUES(@m,@l,@s,@t)",
                    new SqlParameter("@m", ma), new SqlParameter("@l", loai), new SqlParameter("@s", stt), new SqlParameter("@t", tt));
        ...
        public KetQuaXuLy LapDat(string soPhieu, string maTN, string soPhong, DateTime ngay, string tinhTrang, string maNV, string ghiChu)
        {
            ...
            try
            {
                Db.Execute("INSERT INTO PhieuLapDat VALUES(@p,@tn,@ph,@n,@tt,@nv,@g)",
                    ...
                    new SqlParameter("@n", ngay.Date), new SqlParameter("@tt", tinhTrang), new SqlParameter("@nv", maNV), new SqlParameter("@g", ghiChu));
