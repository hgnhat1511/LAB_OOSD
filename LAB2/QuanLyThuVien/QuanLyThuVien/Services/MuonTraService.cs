...
WHERE pm.MaDocGia=@Ma AND ct.NgayTraThucTe IS NULL", new SqlParameter("@Ma", maDocGia)));
            if (dangMuon + soSachMuonMoi > 3) return KetQuaXuLy.Loi("Tổng số sách đang mượn và sắp mượn không được vượt quá 3 cuốn.");
        ...
        public KetQuaXuLy LapPhieuMuon(string maDocGia, string maNhanVien, IList<string> maDauSach, DateTime ngayMuon, DateTime ngayHenTra)
        {
            ...
            foreach (string ma in maDauSach) if (!string.IsNullOrWhiteSpace(ma)) unique.Add(ma.Trim());
            ...
            using (SqlTransaction tx = cn.BeginTransaction(IsolationLevel.Serializable))
            {
                try
                {
                    ...
                    using (SqlCommand insPm = new SqlCommand(@"INSERT INTO PhieuMuon(MaPhieuMuon,MaDocGia,MaNhanVien,NgayMuon,NgayHenTra) VALUES(@MaPM,@MaDG,@MaNV,@NgayMuon,@HenTra)", cn, tx))
                    {
                    ...
                    foreach (string maSach in unique)
                    {
                        ...
                        using (SqlCommand insCt = new SqlCommand(@"INSERT INTO ChiTietPhieuMuon(MaChiTiet,MaPhieuMuon,MaDauSach) VALUES(@MaCT,@MaPM,@MaSach)", cn, tx))
                        {
                        ...
                        using (SqlCommand update = new SqlCommand(@"UPDATE DauSach SET SoLuongHienCo=SoLuongHienCo-1 WHERE MaDauSach=@Ma AND SoLuongHienCo>0", cn, tx))
                        {
                            update.Parameters.AddWithValue("@Ma", maSach);
                            if (update.ExecuteNonQuery() != 1) throw new InvalidOperationException("Không thể cập nhật tồn kho cho " + maSach + ".");
                        }
                        i++;
                    }
                    tx.Commit();
                    return KetQuaXuLy.Ok("Lập phiếu mượn " + maPM + " thành công.");
                }
                catch (Exception ex)
                {
                    try { tx.Rollback(); } catch { }
                    return KetQuaXuLy.Loi("Không thể lập phiếu mượn: " + ex.Message);
                }
            }
        }
        ...
        {
            return Db.Query(@"SELECT ct.MaChiTiet, pm.MaPhieuMuon, pm.MaDocGia, ct.MaDauSach, s.TenSach, pm.NgayMuon, pm.NgayHenTra 
 FROM PhieuMuon pm JOIN ChiTietPhieuMuon ct ON ct.MaPhieuMuon=pm.MaPhieuMuon 
 JOIN DauSach s ON s.MaDauSach=ct.MaDauSach 
 WHERE pm.MaDocGia=@Ma AND ct.NgayTraThucTe IS NULL ORDER BY pm.NgayHenTra, s.TenSach", new SqlParameter("@Ma", maDocGia));
        }
        ...
        public KetQuaXuLy TraSach(string maChiTiet, string maNhanVien, DateTime ngayTra, string tinhTrang, decimal phiPhat)
        {
            ...
            using (SqlTransaction tx = cn.BeginTransaction(IsolationLevel.Serializable))
            {
                try
                {
                    string maSach; DateTime ngayMuon; DateTime ngayHenTra;
                    using (SqlCommand cmd = new SqlCommand(@"SELECT ct.MaDauSach,ct.NgayTraThucTe,pm.NgayMuon,pm.NgayHenTra FROM ChiTietPhieuMuon ct 
 JOIN PhieuMuon pm ON pm.MaPhieuMuon=ct.MaPhieuMuon WHERE ct.MaChiTiet=@MaCT", cn, tx))
                    {
                        ...
                        using (SqlDataReader rd = cmd.ExecuteReader())
                        {
                            ...
                            maSach = rd.GetString(0); ngayMuon = rd.GetDateTime(2); ngayHenTra = rd.GetDateTime(3);
                        }
                    }
                    ...
                    bool huHong = tt.IndexOf("Rách", StringComparison.OrdinalIgnoreCase) >= 0 || tt.IndexOf("Hư", StringComparison.OrdinalIgnoreCase) >= 0;
                    bool canPhat = quaHan || mat || huHong;
                    if (canPhat && phiPhat <= 0) return KetQuaXuLy.Loi("Trường hợp trả trễ/mất/hư hỏng phải nhập phí phạt > 0.");

                    using (SqlCommand up = new SqlCommand(@"UPDATE ChiTietPhieuMuon SET NgayTraThucTe=@NgayTra,TinhTrangTra=@TinhTrang WHERE MaChiTiet=@MaCT AND NgayTraThucTe IS NULL", cn, tx))
                    {
                    ...
                    if (!mat && !huHong)
                    {
                        using (SqlCommand stock = new SqlCommand("UPDATE DauSach SET SoLuongHienCo=SoLuongHienCo+1 WHERE MaDauSach=@Ma", cn, tx))
                        {
                            stock.Parameters.AddWithValue("@Ma", maSach);
                    ...
                    if (canPhat)
                    {
                        ...
                        if (quaHan) lyDo.Add("Trả trễ hạn");
                        if (mat) lyDo.Add("Mất sách");
                        if (huHong) lyDo.Add("Rách/hư hỏng");
                        string maPhat = "PP" + DateTime.Now.ToString("yyyyMMddHHmmssfff");
                        using (SqlCommand phat = new SqlCommand(@"INSERT INTO PhieuPhat(MaPhieuPhat,MaChiTiet,MaNhanVien,NgayPhat,LyDo,PhiPhat) VALUES(@MaPP,@MaCT,@MaNV,@Ngay,@LyDo,@Phi)", cn, tx))
                        {
                            phat.Parameters.AddWithValue("@MaPP", maPhat);
                            phat.Parameters.AddWithValue("@MaCT", maChiTiet);
                            phat.Parameters.AddWithValue("@MaNV", maNhanVien);
                            phat.Parameters.AddWithValue("@Ngay", ngayTra.Date);
                            phat.Parameters.AddWithValue("@LyDo", string.Join(", ", lyDo));
                            phat.Parameters.AddWithValue("@Phi", phiPhat);
                            phat.ExecuteNonQuery();
                        }
                    }
                    tx.Commit();
                    return KetQuaXuLy.Ok(canPhat ? "Trả sách thành công và đã lập phiếu phạt." : "Trả sách thành công.");
                }
                catch (Exception ex)
                {
                    try { tx.Rollback(); } catch { }
                    return KetQuaXuLy.Loi("Không thể xử lý trả sách: " + ex.Message);
                }
            }
        }
    }
