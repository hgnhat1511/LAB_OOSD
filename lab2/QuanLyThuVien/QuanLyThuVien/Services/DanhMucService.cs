...
namespace QuanLyThuVien.Services
{
    public class DanhMucService
    {
        ...
        {
            if (nv == null || string.IsNullOrWhiteSpace(nv.MaNhanVien) || string.IsNullOrWhiteSpace(nv.Ho) || string.IsNullOrWhiteSpace(nv.Ten) || string.IsNullOrWhiteSpace(nv.Phai) || string.IsNullOrWhiteSpace(nv.ChucVu))
                return KetQuaXuLy.Loi("Vui lòng nhập đầy đủ thông tin bắt buộc của nhân viên.");
            try
            {
                string sql = capNhat
                    ? @"UPDATE NhanVien SET Ho=@Ho,Ten=@Ten,Phai=@Phai,NgaySinh=@NgaySinh,ChucVu=@ChucVu,SoDienThoai=@SDT WHERE MaNhanVien=@Ma"
                    : @"INSERT INTO NhanVien(MaNhanVien,Ho,Ten,Phai,NgaySinh,ChucVu,SoDienThoai) VALUES(@Ma,@Ho,@Ten,@Phai,@NgaySinh,@ChucVu,@SDT)";
                ...
                return n > 0 ? KetQuaXuLy.Ok(capNhat ? "Cập nhật nhân viên thành công." : "Thêm nhân viên thành công.") : KetQuaXuLy.Loi("Không có dữ liệu được thay đổi.");
            }
        ...
        {
            if (string.IsNullOrWhiteSpace(ma) || string.IsNullOrWhiteSpace(ten)) return KetQuaXuLy.Loi("Mã và tên thể loại không được để trống.");
            try
            {
                string sql = capNhat ? "UPDATE TheLoai SET TenTheLoai=@Ten WHERE MaTheLoai=@Ma" : "INSERT INTO TheLoai(MaTheLoai,TenTheLoai) VALUES(@Ma,@Ten)";
        ...
        public KetQuaXuLy LuuNhaXuatBan(string ma, string diaChi, string sdt, bool capNhat)
        {
            ...
            try
            {
                string sql = capNhat ? "UPDATE NhaXuatBan SET DiaChi=@DiaChi,SoDienThoai=@SDT WHERE MaNhaXuatBan=@Ma" : "INSERT INTO NhaXuatBan(MaNhaXuatBan,DiaChi,SoDienThoai) VALUES(@Ma,@DiaChi,@SDT)";
                int n = Db.Execute(sql, new SqlParameter("@Ma", ma.Trim()), new SqlParameter("@DiaChi", (object)(diaChi ?? string.Empty)), new SqlParameter("@SDT", (object)(sdt ?? string.Empty)));
