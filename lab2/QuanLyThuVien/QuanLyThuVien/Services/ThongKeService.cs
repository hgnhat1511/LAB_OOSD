    ...
    public class ThongKeService
    {
        public ThongKeTongHop LayTongHop(DateTime tuNgay, DateTime denNgay)
        {
            ...
            kq.LuotSachMuon = Convert.ToInt32(Db.Scalar(@"SELECT COUNT(*) FROM ChiTietPhieuMuon ct 
                JOIN PhieuMuon pm ON pm.MaPhieuMuon=ct.MaPhieuMuon WHERE pm.NgayMuon BETWEEN @Tu AND @Den",
            ...
            kq.SachQuaHan = Convert.ToInt32(Db.Scalar(@"SELECT COUNT(*) FROM ChiTietPhieuMuon ct 
                ...
                OR (ct.NgayTraThucTe IS NOT NULL AND ct.NgayTraThucTe > pm.NgayHenTra AND ct.NgayTraThucTe BETWEEN @Tu AND @Den)",
                new SqlParameter("@Tu", from), new SqlParameter("@Den", to)));
            ...
            kq.SachHuHong = Convert.ToInt32(Db.Scalar(@"SELECT COUNT(*) FROM ChiTietPhieuMuon 
                WHERE NgayTraThucTe BETWEEN @Tu AND @Den AND (TinhTrangTra LIKE N'%Rách%' OR TinhTrangTra LIKE N'%Hư%')",
                new SqlParameter("@Tu", from), new SqlParameter("@Den", to)));
        ...
        {
            return Db.Query(@"SELECT pp.MaPhieuPhat, pp.NgayPhat, pm.MaDocGia, ct.MaDauSach, s.TenSach, pp.LyDo, pp.PhiPhat, pp.MaNhanVien 
                FROM PhieuPhat pp 
